...
import { LeadStatus, TaskPriority } from "../../store/slices/crmSlice";
...
const Home = () => {
  const { leads, tasks, bookings, communications, currency } = useSelector(
    (state) => state.crm
  );
  ...
  const metrics = useMemo(() => {
    ...
    // Fix the itinerary types issue
    const upcomingTrips = bookings
      .filter(
        (booking) =>
          booking.status === "confirmed" &&
          booking.itinerary?.startDate &&
          new Date(booking.itinerary.startDate) > new Date()
      )
      .sort((a, b) => {
        const aDate = a.itinerary?.startDate
          ? new Date(a.itinerary.startDate).getTime()
          : 0;
        const bDate = b.itinerary?.startDate
          ? new Date(b.itinerary.startDate).getTime()
          : 0;
        return aDate - bDate;
      })
      .slice(0, 3);
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        ...
        <div className="flex gap-2">
          ...
          <Button onClick={() => navigate("/crm/tasks/new")}>
            Create Task
          </Button>
        </div>
      </div>
                      ...
                      <div className="text-sm">
                        <div className="flex justify-between items-center">
                          ...
                          <span className="font-medium">
                            {trip.itinerary?.startDate
                              ? format(
                                  new Date(trip.itinerary.startDate),
                                  "MMM d, yyyy"
                                )
                              : "TBD"}
                          </span>
                        ...
                        <div className="flex justify-between items-center mt-1">
                          ...
                          <span className="font-medium">
                            {trip.itinerary?.endDate
                              ? format(
                                  new Date(trip.itinerary.endDate),
                                  "MMM d, yyyy"
                                )
                              : "TBD"}
                          </span>
                        ...
                        <div className="mt-3">
                          <Button
                            ...
                            className="w-full"
                            onClick={() =>
                              navigate(`/crm/leads/${trip.leadId}`)
                            }
                          >
                            View Details
                          </Button>
