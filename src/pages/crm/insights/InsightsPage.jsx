...
const InsightsPage = () => {
  const { leads, communications, tasks, bookings, isOffline } = useSelector(
    (state) => state.crm
  );

  // Generate basic cohorts (simplified since we don't have the AI utility)
  const cohorts = useMemo(() => {
    ...
    // Return cohorts with at least 2 leads
    return Object.values(tagGroups).filter(
      (cohort) => cohort.leadIds.length >= 2
    );
  }, [leads]);
